...
function sixs_image_sitemap_generate()
{
	...
	echo '<p>' . __('After the sitemap has been created, you can submit it using <a href="https://search.google.com/search-console" target="_blank">Google Search Console</a> .', 'simple-xml-image-sitemap') . '</p>';
	echo '</div>';
		...
		if (wp_verify_nonce($_POST['nonce'], 'sixs_image_sitemap_generate')) {
			...
			switch (sixs_image_sitemap_create()) {
				...
				default:
					...
					$url = $url['scheme'] . '://' . $url['host'] . '/image-sitemap.xml';
...
function sixs_image_sitemap_create()
{
	...
	$posts = $wpdb->get_results("SELECT * FROM $wpdb->posts WHERE post_type<>'revision' AND post_type IN ('post', 'page', 'attachment') AND post_status IN ('publish','inherit') order by post_date DESC");
	$thumbs = $wpdb->get_results("SELECT post_name, post_mime_type, guid, post_excerpt, post_title, post_parent FROM wp_posts WHERE post_type = 'attachment'");
			...
			foreach ($images as $k => $v) {
				...
				if (!empty($permalink)) {
					...
					foreach (array_keys($imgurls) as $imgurl) {
						...
						$guids = $wpdb->get_results("SELECT post_name, post_mime_type, guid, post_excerpt, post_title, post_content, post_parent
									FROM $wpdb->posts
						...
						*/
						foreach ($guids as $guid) {
							...
							$img .= '       <image:image>' . "\n";
							$img .= '       	   <image:loc>' . $guid->guid . '</image:loc>' . "\n";
							$img .= '       	   <image:caption>' . esc_html($guid->post_excerpt) . '</image:caption>' . "\n"; // text below picture
							$img .= '        	   <image:title>' . esc_html($guid->post_content) . '</image:title>' . "\n"; // meta data image title
							$img .= '       </image:image>' . "\n";
						}
					}
	...
	$image_sitemap_url = $_SERVER["DOCUMENT_ROOT"] . '/image-sitemap.xml';
...
if (file_exists($_SERVER["DOCUMENT_ROOT"] . '/image-sitemap.xml')) {
