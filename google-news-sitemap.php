...
/*
	Plugin Name: Andrea Pernici News Sitemap for Google
	Plugin URI: https://andreapernici.com/wordpress/google-news-sitemap/
	Description: Automatically generate sitemap for inclusion in Google News. Go to <a href="options-general.php?page=apgnsm.php">Settings -> Andrea Pernici News Sitemap for Google</a> for setup.
	Version: 1.0.14
	...
	*/
	...
	function apgnsm_add_pages() {
		add_options_page("Andrea Pernici News Sitemap for Google", "Sitemap Google News", 'edit_posts', basename(__FILE__), "apgnsm_admin_page");
	}
	...
	function apgnsm_permissions() {
		...
		$apgnsm_news_file_path = $apgnsm_path . "news-sitemap.xml";

		$apgnsm_permission = 0;

		if ($apgnsm_news_active && is_file($apgnsm_news_file_path) && is_writable($apgnsm_news_file_path))  {
			$apgnsm_permission += 0;
		} elseif ($apgnsm_news_active && !is_file($apgnsm_news_file_path) && is_writable($apgnsm_path)) {
			$fp = fopen($apgnsm_news_file_path, 'w');
			fwrite($fp, "<?xml version=\"1.0\" encoding=\"UTF-8\"?><urlset xmlns=\"https://www.sitemaps.org/schemas/sitemap/0.9\" xmlns:n=\"https://www.google.com/schemas/sitemap-news/0.9\" />");
			fclose($fp);
			if (is_file($apgnsm_news_file_path) && is_writable($apgnsm_news_file_path)) {
				$apgnsm_permission += 0;
			} else {
				$apgnsm_permission += 2;
			}
		} elseif ($apgnsm_news_active) {
			$apgnsm_permission += 2;
		}
		else {
			$apgnsm_permission += 0;
	...
	*/
		...
		*/
		...
		//mysql_select_db(DB_NAME);
			...
			$posts = $wpdb->get_results("SELECT * FROM ".$wpdb->posts." WHERE `post_status`='publish'
				AND post_modified_gmt >= ( NOW() - INTERVAL 48 HOUR ) AND (`post_type`='page' OR `post_type`='post') ". $includeNoCat . ' ' . $includeNoPost." GROUP BY `ID` ORDER BY `post_modified_gmt` DESC");
	...
	if ($apgnsm_news_active && $apgnsm_permission != 2) {
		$fp = fopen(ABSPATH . $apgnsm_path . "news-sitemap.xml", 'w');
	...
	if ((time() - $apgnsm_last_ping) > 60 * 60) {
			//get_headers("https://www.google.com/webmasters/tools/ping?sitemap=" . urlencode($home . $apgnsm_path . "sitemap.xml"));	//PHP5+
		$fp = @fopen("https://www.google.com/webmasters/tools/ping?sitemap=" . urlencode($home . $apgnsm_path . "news-sitemap.xml"), 80);
		@fclose($fp);
...
function apgnsm_admin_page() {
	...
	$apgnsm = isset( $_POST['apgnsm_submit'] ) ? filter_input( INPUT_POST, 'apgnsm_submit' ) : null;

	// Check form submission and update options
	if ( 'apgnsm_submit' == $apgnsm ) {
...
if ($apgnsm_permission == 1) $msg = "Error: there is a problem with <em>news-sitemap.xml</em>. It doesn't exist or is not writable. <a href=\"https://www.andreapernici.com/wordpress/google-news-sitemap/\" target=\"_blank\" >For help see the plugin's homepage</a>.";
elseif ($apgnsm_permission == 2) $msg = "Error: there is a problem with <em>news-sitemap.xml</em>. It doesn't exist or is not writable. <a href=\"https://www.andreapernici.com/wordpress/google-news-sitemap/\" target=\"_blank\" >For help see the plugin's homepage</a>.";
elseif ($apgnsm_permission == 3) $msg = "Error: there is a problem with <em>news-sitemap.xml</em>. It doesn't exist or is not writable. <a href=\"https://www.andreapernici.com/wordpress/google-news-sitemap/\" target=\"_blank\" >For help see the plugin's homepage</a>.";
?>
...
<div class="wrap" id="sm_div">
	<h2>Andrea Pernici News Sitemap for Google</h2>
	by <strong>Andrea Pernici</strong>
	<p>
		&nbsp;<a target="_blank" title="Google News Sitemap Plugin Release History" href="https://andreapernici.com/wordpress/google-news-sitemap/">Changelog</a>
		| <a target="_blank" title="Google News Sitemap Support" href="https://andreapernici.com/wordpress/google-news-sitemap/">Support</a>
	</p>
	<?php	if ($msg) {	?>
	...
	<div style="width:824px;">
		<div style="float:left;background-color:white;padding: 10px 10px 10px 10px;margin-right:15px;border: 1px solid #ddd;">
			<div style="width:350px;height:130px;">
				<h3>Donate</h3>
				<em>If you like this plugin and find it useful, help keep this plugin free and actively developed by going to the <a href="https://andreapernici.com/donazioni" target="_blank"><strong>donate</strong></a> page on my website.</em>
				<p><em>Also, don't forget to follow me on <a href="https://twitter.com/andreapernici/" target="_blank"><strong>Twitter</strong></a>.</em></p>
			</div>
		</div>
		...
		<div style="float:left;background-color:white;padding: 10px 10px 10px 10px;border: 1px solid #ddd;">
			<div style="width:415px;height:130px;">
				...
				<p><em>For any doubt refer to google guidelines <a href="https://www.google.com/support/webmasters/bin/answer.py?hl=en&answer=74288">here</a>.</em></p>
				<p><em>Plugin by <a href="https://www.andreapernici.com">Andrea Pernici</a> with support of <a href="https://www.ghenghe.com/">Ghenghe Social Bookmark</a> and <a href="https://www.ciakprestitiemutui.com/">Ciak Prestiti e Mutui</a>. We would also like to recommend <a href="https://www.convegnogt.it/">Convegno Gt</a> to discover new important tricks for Google News Ranking.</em> </p>
			</div>
		</div>
	</div>
			...
			<div class="inner-sidebar">
				<div id="side-sortables" class="meta-box-sortabless ui-sortable" style="position:relative;">
					<div id="sm_pnres" class="postbox">
						...
						<div class="inside">
							<a class="sm_button sm_pluginHome"    href="https://www.andreapernici.com/wordpress/google-news-sitemap/">Plugin Homepage</a>
							<a class="sm_button sm_pluginList"    href="https://www.google.com/support/webmasters/bin/answer.py?hl=en&answer=74288">Google News Guidelines</a>
							<a class="sm_button sm_pluginSupport" href="https://wordpress.org/extend/plugins/google-news-sitemap/">Forum</a>
							<a class="sm_button sm_donatePayPal"  href="https://andreapernici.com/donazioni">Donations</a>
						</div>
					</div>
				</div>
			</div>
						...
						<div id="sm_rebuild" class="postbox">
							...
							<div class="inside">
								...
								<form name="form1" method="post" action="<?php echo $_SERVER["REQUEST_URI"]; ?>&amp;updated=true">
									...
									<b>Advanced settings - default sitemap will be generated in https://yourdomain.com/news-sitemap.xml</b>
									<ul>
										<li>
